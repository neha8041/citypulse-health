...
class MorningBriefingWorkflow:
    ...
    def generate_briefing(self) -> dict:
        try:
            ...
            return {
                "morning_briefing": result.get(
                    "summary", "Briefing could not be generated."
                ),
                "explanation": result.get("explanation", ""),
                "actions": result.get("actions", []),
                "anomalies": live_anomalies.get("anomalies", [])
                if isinstance(live_anomalies, dict) else [],
                "summary": live_summary if isinstance(live_summary, dict)
                and "error" not in live_summary else {"error": "Failed to load summary"}
            }
        except Exception as e:
            logging.error("Failed to generate briefing: %s", e)
            return {
                "morning_briefing": (
                    "Systems are currently operating in fallback mode due to a "
                    "temporary service interruption. Core health data is being actively monitored."
                ),
                "explanation": (
                    "We encountered an issue connecting to our intelligence systems. "
                    "This is usually due to missing permissions or API quotas."
                ),
                "actions": [
                    "Verify IAM permissions for Vertex AI and BigQuery",
                    "Check Cloud Run logs for detailed trace"
                ],
                "anomalies": [],
